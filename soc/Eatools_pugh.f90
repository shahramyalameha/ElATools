...
SUBROUTINE Cpugh(Minpughvar,Maxpughvar,pughvar_ave,phi,theta,k11,k12,k13,k22,k23,k33,a6666,sheainvar)
        
	  DOUBLE PRECISION, PARAMETER                    ::  pi=3.1415926535897932384626433832795d0
	  DOUBLE PRECISION, DIMENSION(6,6)               ::  C=0D0,S=0D0,CP=0D0
	  DOUBLE PRECISION, DIMENSION(3)                 ::  k=0D0,v=0D0
	  DOUBLE PRECISION                               ::  a6666, sheainvar,Minpughvar,Maxpughvar,pughvar_ave,gamma,BINver,BB,&
	...
	DO kk=0,Nmesh_gamma-1      
		...
		BB=( S(1,1)+S(1,2)+S(1,3) )*k11&
           +( S(1,6)+S(2,6)+S(3,6) )*k12&
           +( S(1,5)+S(2,5)+S(3,5) )*k13&
           +( S(1,2)+S(2,2)+S(2,3) )*k22&
           +( S(1,4)+S(2,4)+S(3,4) )*k23&
...
Minpughvar   = Maxsheainvar /BINver
Maxpughvar   = Minsheainvar /BINver
pughvar_ave  = sheainvar_ave/BINver
