!```````````````````````````````````````````````````````````````````````````````````````````````
! Copyright (c) 2017-2022 Shahram Yalameha <yalameha93@gmail.com> , <sh.yalameha@sci.ui.ac.ir>, `
...
SUBROUTINE plotRatio3d(YN,p)
   ...
   IF (YN.eq."Y")THEN
    ...
    if (p==1) then
      ...
      WRITE(35,"(a)")'set output "Poissons_ratio.pdf"'
      ...
      WRITE(35,"(a)")'set palette defined (-1 "green", 0 "white",1 "red" )'
      ...
      WRITE(35,"(a)")'splot "3d_poissons.dat"  u 1:2:3 w pm3d title "Max. positive"'
      WRITE(35,"(a)")'splot "3d_poissons.dat"  u 4:5:6 w pm3d title "Min. positive"'
      WRITE(35,"(a)")'splot "3d_poissons.dat" u 7:8:9 w pm3d title "Negative"'
