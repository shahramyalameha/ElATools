...
!                                                                                          `
!```````````````````````````````````````````````````````````````````````````````````````````
! SUBROUTINE: For 3D matereials; 3d surface map : data file ( of AAEP code) to html file.

subroutine threeddmaphtmlmod()
   implicit none
   DOUBLE PRECISION, DIMENSION(100000)  :: G_max,shminp,shminn,shavep,SINver,CO,comminp,pugh_max,pughminp,pughminn,pughavep,&
   ...
   do ii = 1, (n_phif*n_thetaf)+n_phif
      ! print*, ii
...
!-----------------------------   
   ...
   do iii = 1, (n_phif*n_thetaf) + n_thetaf + 1
...
!-----------------------------      
   ...
   open (13, file=".SDdat")
   do j = 1, (n_phif*n_thetaf) + n_thetaf

      IF (ynveloc0 == 'Y' .OR. ynveloc0 == 'y') THEN
!===============================First point>
         IF (j == 1) then
            DO st = 1, n_phif+1
                     Write (13, "(38F25.15)") theta(j), phi(st),G_max(j),shminp(j),shminn(j),shavep(j),SINver(j),CO(j),comminp(j),&                  !9
...
subroutine sp_neg_valhmap(val, ngpos)
   implicit none
   ...
   if (val == "hmpoi") ngpos = Pratio_min
   if (val == "hmcomp") ngpos = Mincomp
